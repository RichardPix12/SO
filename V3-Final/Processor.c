...
void Processor_DecodeAndExecuteInstruction() {
	...
	switch (operationCode) {
		...
		case OS_INST: // Make a operating system routine in entry point indicated by operand1
			...
			if(Processor_PSW_BitState(EXECUTION_MODE_BIT)){
				ComputerSystem_DebugMessage(130, HARDWARE, OperatingSystem_GetExecutingProcessID(), operationCode, operand1, operand2, registerPC_CPU, registerAccumulator_CPU, registerPSW_CPU, Processor_ShowPSW());
		...
		// Instruction MEMADD
		case MEMADD_INST:
			//Pongo los operandos en sus registro
			registerAccumulator_CPU = operand1; //Registro acumulador porque ya es directo
			registerMAR_CPU = operand2; // Registro MAR porque queremos leer de una posición de memoria
			
			//Busco los datos en memoria
			
			Buses_write_AddressBus_From_To(CPU, MMU);// Paso la direccion			
			registerCTRL_CPU=CTRLREAD;
			Buses_write_ControlBus_From_To(CPU,MMU);				// Regsitro control a leer
			
			
			//Lo añado al acumulador e incremento el contador de programa
			registerAccumulator_CPU += registerMBR_CPU.cell; //Leo de memoria y lo sumo al resultado
			registerPC_CPU++;
			break;
